...
Batch transcription now runs through asynchronous tasks with observable stages,
bounded GPU concurrency, result retrieval, and cooperative cancellation. See
`PHASE_4_PLAN.md` and `docs/ASYNC_TASK_API_GUIDE.md`.

The Phase 5A streaming foundation adds WebSocket sessions, binary audio chunk
framing, bounded buffering, ordering checks, and backpressure signals. See
`PHASE_5A_PLAN.md` and `docs/STREAMING_PROTOCOL.md`.

Phase 5B adds background stream processing, a shared GPU scheduler, sliding
...
When a live microphone session stops, the backend performs an offline
second-pass refinement and sends an explicit transcript revision with improved
...
Phase 6 adds remote HTTP provider options for offline transcription, speaker
diarization, and live ASR. API-only cloud deployments can use
`backend/requirements-cloud.txt` without installing local model runtimes. See
`PHASE_6_PLAN.md` and `docs/REMOTE_MODEL_API_GUIDE.md`.

The live microphone path can also connect directly to Volcengine Doubao
